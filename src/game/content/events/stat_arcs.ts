...
export const STAT_ARC_EVENTS: EventFamily[] = [
  ...
  beat({
    ...
    weight: 0,
    title: '你开始把落灰听成脚步',
    body: '每落一粒都像有人在上一层轻轻走。你数到十七的时候停了，因为十七是你门牌。',
    choices: [
      ch('write', '写下来：这是灰，不是人', { stats: { sanity: 4 }, log: '纸上的字比脑子稳。你把它贴在门上。', tone: 'good' }),
      ch('wait', '等到真的有脚步再反应', { stats: { sanity: -3 }, world: { exposure: 2 }, log: '你等了一夜。没有人。灰还在落。', tone: 'neutral' }),
  ...
  beat({
    ...
    choices: [
      ...
      ch('eat', '强迫自己吃一口', { res: { foodStaple: -1 }, stats: { sanity: 3, hp: 0 }, log: '咸的。嘴里有味道之后，这间屋子才又像真的。', tone: 'neutral' }, { requires: { res: { foodStaple: 1 } } }),
  ...
  beat({
    ...
    weight: 0,
    title: '上楼的时候你在半层坐下来',
    body: '不是单纯想歇。腿先软了，人跟着坐下。心跳比你记得的响。\n药箱还在原来的位置。',
    choices: [
      ch('slow', '承认今天只能慢', { stats: { sanity: 2, stamina: 4 }, setFlags: ['flag:admittedHurt'], schedule: [{ familyId: 'stat_arc_hp_2', waitFor: 'treat' }], log: '你把步子改小。小步也能到家。', tone: 'good' }),
  ...
  beat({
    ...
    choices: [
      ...
      ch('tear', '撕掉，当它没写过', { stats: { sanity: -4 }, log: '纸很轻。撕开的声音比字吓人。', tone: 'grim' }),
  ...
  beat({
    ...
    weight: 0,
    title: '背包刚拎起来，肩就先软了',
    body: '包没有比昨天更重。是你没有力气了。你把包放下，又拎起来，又放下。光是这样就已经喘。',
    choices: [
  ...
  beat({
    ...
    choices: [
      ch('nod', '点头，表示你懂', { stats: { sanity: -2, humanity: 2 }, setFlags: ['flag:seenFlinch'], schedule: [{ familyId: 'stat_arc_humanity_2', inDays: 2 }], log: '你点头。他们走得更快。懂了也还是受伤。', tone: 'grim' }),
      ch('speak', '说一句"我没有恶意"', { stats: { reputation: 1, sanity: -3 }, log: '声音在楼梯间发空。没有人回答。', tone: 'neutral' }),
  ...
  beat({
    ...
    once: true,
    weight: 0,
    title: '你发现自己计算别人值不值得帮，用时不到一秒',
    body: '以前你会为这个速度愧疚。现在你只是确认：算完了。答案是不。',
    choices: [
      ch('hate', '把这个速度当成病', { stats: { humanity: 3, sanity: -2 }, log: '你讨厌自己。讨厌至少说明旧标准还在。', tone: 'good' }),
      ch('keep', '承认这是新标准', { stats: { humanity: -3, sanity: 2 }, log: '你没有改答案。一秒已经够用。', tone: 'grim' }),
      skip('你去洗手。洗不掉刚才那一下计算。', { stats: { stamina: -2 } }),
    ],
  }),
  beat({
    id: 'stat_arc_rep_1',
  ...
  beat({
    ...
    choices: [
      ch('keep', '把字条夹进日记', { stats: { sanity: 2, reputation: 1 }, log: '两句话都留下。你需要这种不绕弯的东西。', tone: 'neutral' }),
  ...
  beat({
    ...
    title: '灯关着的时候，屋子里的声音重新排队',
    body: '冰箱不响了。管道还在。楼上有人走动，以前被灯的嗡嗡盖住。\n眼睛在适应。适应之后你开始觉得阴影里有形状。',
    choices: [
      ch('candle', '点一支蜡烛，挡严窗帘', { stats: { sanity: 3 }, world: { exposure: 1 }, setFlags: ['flag:darkCandle'], schedule: [{ familyId: 'stat_arc_dark_2', inDays: 2 }], log: '火很小。窗帘比火重要。', tone: 'good' }),
      ch('sit', '在黑暗里坐到习惯', { stats: { sanity: -3, stamina: 4 }, schedule: [{ familyId: 'stat_arc_dark_2', waitFor: 'setPowerPriority' }], log: '你坐着。后来不怕了，只是眼睛酸。', tone: 'neutral' }),
      skip('你把供电表翻出来，想把灯排上去。', { stats: { sanity: 1 } }),
    ],
  }),
  beat({
    id: 'stat_arc_dark_2',
    ...
    weight: 0,
    title: '黑暗里有人问你还亮不亮灯',
    body: '问句从窗外进来。你听不清是对面楼还是自己在问。答亮或答不亮，都会有人知道这间屋子有人。',
    choices: [
      ch('no', '说不亮', { stats: { sanity: 2 }, world: { exposure: -2 }, log: '你说了。窗外没有第二句。', tone: 'good' }),
      ch('yes', '说亮，然后立刻后悔', { stats: { sanity: -2 }, world: { exposure: 4 }, log: '你说了。窗帘缝里有人停了一下。', tone: 'bad' }),
      skip('你不回答。问句过了一会儿自己停了。', { stats: { stamina: 4, sanity: -1 } }),
    ],
  }),
];
