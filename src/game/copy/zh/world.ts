import { registerTree } from '../t';

export const data = {
  "site": {
    "apartment": {
      "name": "自家公寓",
      "codename": "HOME-6F",
      "desc": "你住了六年的六楼两居室。防盗门是租房时就有的老货，窗外是整座城市。你知道哪块地板会响，也知道楼下便利店几点上货。",
      "pros": [
        "市中心，搜刮效率 +20%",
        "邻居社区已有交情",
        "外出不耗燃料"
      ],
      "cons": [
        "楼板承重有限，水箱上限很低",
        "加固门最多到 2 级",
        "停电后每次外出都要爬六层",
        "窗户太多，藏不住"
      ]
    },
    "bungalow": {
      "name": "老城平房",
      "codename": "YARD-01",
      "desc": "城南待拆迁的一片平房，你姑妈留下的。院子里有口废弃的压水井和一棵石榴树。墙是空心砖，一脚能踹穿，但地是你的。",
      "pros": [
        "有院子，可挖地窖、可大面积种植",
        "水箱容量 +20%",
        "自带 1 级菜圃",
        "可打井取水"
      ],
      "cons": [
        "空心砖墙，加固门上限 2 级",
        "门窗多，正面暴露",
        "声音传得远"
      ]
    },
    "garage": {
      "name": "地下车库 B2",
      "codename": "SUB-B2",
      "desc": "小区地下负二层的一个角落，物业主任姓陈，抽烟。这里没有一丝自然光，常年十六度，混凝土顶板厚得能挡住很多东西——包括信号，也包括逃生的机会。",
      "pros": [
        "混凝土顶板：天然辐射屏蔽",
        "恒温，自带 2 级保温层层",
        "隐蔽性极佳，暴露度基础值最低",
        "加固门可达 3 级"
      ],
      "cons": [
        "没有自然光，菜圃必须靠电补光",
        "通风极差，空气过滤器是刚需",
        "内涝会致命",
        "电台最多 1 级"
      ],
      "reason": "需要谈判 2 级说服物业"
    },
    "farmhouse": {
      "name": "郊区农舍",
      "codename": "RURAL-7",
      "desc": "离城四十公里，你爸的老宅。有井、有地、有个塌了一半的猪圈。最近的邻居在两公里外。这里没人会来打劫你——因为也没人会来救你。",
      "pros": [
        "有水井，自带净水器与水箱",
        "大片土地，菜圃可达 3 级",
        "远离人群，几乎不会被袭击",
        "水箱容量 +50%"
      ],
      "cons": [
        "搜刮产出只有 70%，且每趟烧 2.5 L 油",
        "孤立无援，没有邻居预警",
        "冬天更冷",
        "搬迁需要车"
      ],
      "reason": "需要一辆车才能搬过去"
    },
    "bunker": {
      "name": "废弃人防工程",
      "codename": "CD-1971",
      "desc": "七十年代挖的防空洞，入口藏在一间关了很久的修车铺后面。铁门锈住了，里面滴水，墙上还留着褪色的标语。它为一场没有发生的战争而建，现在终于要派上用场。",
      "pros": [
        "自带 2 级加固门，遮光帘满级",
        "几乎不可能被发现",
        "厚土层，对辐射和尘暴都有效"
      ],
      "cons": [
        "潮湿，有霉菌性肺病风险",
        "无信号，电台需要架外置天线",
        "入口修复要先花 10 零件",
        "发电机上限只有 2 级"
      ],
      "reason": "锈死的闸门与通风机需要 10 零件才能修复"
    },
    "watertower": {
      "name": "山腰水塔",
      "codename": "TOWER-N",
      "desc": "北山半坡上一座停用的市政水塔。爬上去要走三百级铁梯，但站在顶上，你能看见整条进城的公路——以及任何朝你走来的人。",
      "pros": [
        "满级水箱，容量翻倍",
        "视野极佳：所有袭击都会提前一天预警",
        "重力供水，净水器不耗电",
        "电台信号极好"
      ],
      "cons": [
        "空间狭小，最多只能收留 1 人",
        "保温层上限 1 级，风雪直接打在铁皮上",
        "搜刮产出只有 60%",
        "每天上下山极耗体力"
      ],
      "reason": "需要车才能把物资运上山"
    }
  },
  "module": {
    "fortify": {
      "name": "加固门",
      "short": "门",
      "desc": "门窗、承重、射击位。决定袭击能不能被挡在外面。",
      "zero": "一道租房自带的防盗门，门框是旧木头，锁芯松松垮垮，猫眼后面那块玻璃一敲就碎。",
      "buildPenaltyDesc": "门窗已被拆开做框架，此时被袭击的损失会更大。",
      "level": {
        "1": "门后顶上一根钢管顶杆，两端落点凿进地面与门框；窗框用角铁加固，外面钉死一层建筑模板。挡得住一脚，挡不住撬棍。",
        "2": "换成带钢骨架的防爆门，门框灌了水泥；一楼窗户外焊上螺纹钢花栏，栏后码沙袋。正面撞击只能让门嗡嗡响。",
        "3": "双层门斗：外门防爆、内门实心，中间只留侧身通过的过渡间；门上开观察孔与内侧射击位。撞不开，也围不透。",
      }
    },
    "conceal": {
      "name": "遮光帘",
      "short": "帘",
      "desc": "让别人根本不知道这里有人。降低每日暴露度累积。",
      "zero": "一块旧床单挂在窗帘杆上，风一吹就掀开一角。",
      "buildPenaltyDesc": "脚手架和材料堆在门口，施工期间反而更显眼。",
      "level": {
        "1": "遮光布封死主要窗缝，外面再压两层旧毛毯；灯只在里屋点。夜里从楼下看不出这户有人。",
        "2": "窗户外侧贴了单向镀膜；门面做出「已搬空」的样子——门口堆杂物，贴断水断电的封条，白天看是个空屋。声音也做了隔音。",
        "3": "假入口与真入口分开：走廊那扇门永远锁着、积灰，真正的入口藏在储物间后面；烟道与热源也做了遮挡。",
      }
    },
    "cistern": {
      "name": "水箱",
      "short": "水",
      "desc": "决定你能囤几天的水。容量按日耗算，不是无限水箱。",
      "zero": "几个空矿泉水瓶和一个浴缸。洗菜水留着冲厕所。",
      "buildPenaltyDesc": "水箱正在排空清洗，这段时间净化出来的水没处存。",
      "level": {
        "1": "食品级储桶架在浴缸上方，接一根软管分流；桶盖打了孔通气，底下垫砖防潮。够撑一阵子。",
        "2": "不锈钢水塔架在阳台，装了浮球阀与雨水分流管，出水口前加一道粗滤。雨大的日子能存下不少。",
        "3": "大容量储罐分三层：沉淀、储水、循环；循环泵把末端的水抽回去重新过滤，罐体埋在阴面避光防藻。",
      }
    },
    "filter": {
      "name": "净水器",
      "short": "滤",
      "desc": "雨雪天把水接进桶；旱天靠回用少喝一点。升级提高雨日产量、减缓滤芯损耗。",
      "zero": "一口烧水壶，一个搪瓷盆。泥沙靠沉淀，细菌靠煮沸。",
      "buildPenaltyDesc": "管路断开，施工期间完全无法净水。",
      "level": {
        "1": "两只塑料桶架出落差，中间插一根重力式陶瓷滤芯，出水口再塞一包活性炭。不用电，靠水自己往下滴。",
        "2": "增压泵把水顶过三级串联滤芯——沉棉、颗粒炭、压缩炭，末端一盏紫外灯。雨日接得满，旱天把用过的水回流再滤一遍。",
        "3": "反渗透机组：膜前多介质过滤保护，膜后配压力罐；灰水从回用口抽回来重新过膜。黑雨落下来，也能滤出能喝的一截。",
      }
    },
    "power": {
      "name": "发电机",
      "short": "机",
      "desc": "一切电动模块的前提。也是最响的那个东西。",
      "zero": "几节干电池和一支手电筒，外加一块快没电的充电宝。",
      "buildPenaltyDesc": "线路正在改接，施工期间全屋断电。",
      "level": {
        "1": "两块 200W 单晶板斜架在窗外，线束顺窗框进屋，末端挂一块铅酸电池。够点灯、给手机充电，带不动大功率的东西。",
        "2": "组串式光伏铺满采光最好的一面，接 MPPT 控制器与磷酸铁锂储能柜。晴天有富余，能同时供净水、照明和冰箱。",
        "3": "光伏阵列扩到满负荷，储能柜加厚；旁边并一台柴油发电机组，接自动切换开关。要烧油、要排气、而且很响，但停电时你还亮着灯。",
      }
    },
    "insulate": {
      "name": "保温层",
      "short": "温",
      "desc": "决定屋里的热能留多久。漏得慢，炉子就不用通宵烧。",
      "zero": "关紧窗户，一家人缩在一间屋里。热量从墙缝里往外走。",
      "buildPenaltyDesc": "墙体开了洞，保温效果暂时归零。",
      "level": {
        "1": "窗缝贴密封条，门上挂厚棉帘；只留一间「核心房」取暖，其余房间关死。热量跑得慢了些。",
        "2": "内墙贴聚苯板，窗户改成双层中空，烟道包了保温棉。取暖器不再对着空气白烧。",
        "3": "全屋气密处理，换气走带热回收的机组，墙体与天花板做了隔汽层。屋里的热基本只从门缝走。",
      }
    },
    "airFilter": {
      "name": "空气过滤器",
      "short": "气",
      "desc": "挡灰、毒气、放射性微粒和飞沫。口罩不够用的时候靠它。",
      "zero": "一包一次性口罩。",
      "buildPenaltyDesc": "通风口敞开着，外面的空气正在直接进来。",
      "level": {
        "1": "一台 HEPA 净化器摆在里屋，门窗缝贴了密封条。落灰和一部分放射性微粒被挡在外面。",
        "2": "正压送风：室外空气先过粗滤与活性炭箱再进屋，屋里压强略高于外面，脏空气进不来。",
        "3": "NBC 级过滤机组接管全屋新风，入口做了气密门斗与洗消段。放射性微粒大多被挡在门外。",
      }
    },
    "medbay": {
      "name": "医疗站",
      "short": "医",
      "desc": "减轻病情日损，提高自愈；重病需要更高等级才能用药。三级时主动休息能回一点生命。",
      "zero": "一个家用急救箱，几卷纱布和半瓶碘伏。",
      "buildPenaltyDesc": "医疗物资全部打包着，急救效率大幅下降。",
      "level": {
        "1": "一张专用处置台，消毒区与生活区分开；药品按用途分类放进柜子，需要冷藏的单独隔出。",
        "2": "缝合与固定器械齐全，输液架、氧气袋、简易监护。有电时能做一些原本非要去医院的事。",
        "3": "可做小手术的隔离处置间：无影灯、器械台、器械消毒锅，还有一张术后观察床。",
      }
    },
    "garden": {
      "name": "菜圃",
      "short": "菜",
      "desc": "唯一能凭空产出食物的东西。慢，但它是通往长期生存的门。",
      "zero": "窗台上两个花盆，种什么都不太活。",
      "buildPenaltyDesc": "土全翻开了，本轮没有任何产出。",
      "level": {
        "1": "育苗盘加几个栽培箱，种短周期的叶菜与豆芽；白天搬到采光最好的窗边。",
        "2": "立体水培架接上营养液循环，配一盏补光灯。阴面的屋子也能有一茬收成。",
        "3": "规模栽培：多层架、蘑菇房、种子库分开，产量稳定，阴天与污染天有备用光。",
      }
    },
    "radio": {
      "name": "电台",
      "short": "讯",
      "desc": "外面在发生什么。谁还活着。往哪走。",
      "zero": "一支手摇收音机，信号靠运气。",
      "buildPenaltyDesc": "天线拆下来了，收不到任何东西。",
      "level": {
        "1": "短波天线拉上阳台，手摇机换成电池供电。夜里能收到几个台。",
        "2": "业余收发机接上定向天线，能听也能说；换台要手动调，但能摸到别人的动静。",
        "3": "定向天线阵加频谱扫描，能连续盯着几个频段。情报与预报比谁都早一步。",
      }
    }
  },
  "condition": {
    "thirst": {
      "name": "口渴",
      "desc": "喉咙发紧，尿色变深。水还够维持身体运转——只是不够舒服。"
    },
    "dehydrationMild": {
      "name": "轻度脱水",
      "desc": "嘴唇起皮，站起来有点头晕。身体开始向不重要的器官回收水分。"
    },
    "dehydrationMod": {
      "name": "中度脱水",
      "desc": "站起来眼前发黑，尿液是深黄色的。思考变得像隔着一层棉花。"
    },
    "dehydrationSevere": {
      "name": "重度脱水",
      "desc": "皮肤捏起来不会回弹，心脏在干涸的血管里空转。今晚之前必须有水，否则过不去这一夜。"
    },
    "starving": {
      "name": "饥饿",
      "desc": "胃已经不疼了，那更糟——身体开始烧自己。"
    },
    "malnourished": {
      "name": "营养不良",
      "desc": "牙龈在出血，伤口不愈合。你还站得住，但身体已经在慢慢关掉一些功能。"
    },
    "dysentery": {
      "name": "痢疾",
      "desc": "你喝了不该喝的水。现在身体正在把好不容易存下的水全部排出去。"
    },
    "giardia": {
      "name": "肠寄生虫",
      "desc": "肚子一阵一阵地绞。滤过的水也不保险，卵已经在里面了。"
    },
    "jaundice": {
      "name": "黄疸",
      "desc": "眼白发黄，皮肤也跟着黄。镜子里的自己像换了一层纸。"
    },
    "flu": {
      "name": "流感",
      "desc": "发烧，骨头缝里疼。在有暖气和医院的时代这只是一周的假。"
    },
    "pneumonia": {
      "name": "肺炎",
      "desc": "呼吸浅，咳出来的东西带着颜色。躺着也喘。"
    },
    "wound": {
      "name": "伤口",
      "desc": "手背裂开一道口子。不深，但在脏地方干活，过夜会渗。"
    },
    "woundInfection": {
      "name": "伤口感染",
      "desc": "边缘红肿发烫，有一道红线正沿着手臂往上走。你知道那条线意味着什么。"
    },
    "sepsis": {
      "name": "败血症",
      "desc": "全身发冷又发热。伤口本身已经不重要了——感染进了血。"
    },
    "fracture": {
      "name": "骨折",
      "desc": "固定得再好，它也需要六周。而你没有六周。"
    },
    "hypothermiaMild": {
      "name": "低温症（轻）",
      "desc": "手指发僵，说话有点含糊。屋里再暖一点就会好。"
    },
    "hypothermiaMod": {
      "name": "低温症（中）",
      "desc": "牙齿对不上，穿衣服要试两次。药没用，得把炉子烧起来。"
    },
    "hypothermiaSevere": {
      "name": "低温症（重）",
      "desc": "你已经不觉得冷了。这是最危险的阶段。明天屋里若还低于生存线，就过不去这一夜。"
    },
    "radiationSickness": {
      "name": "辐射病",
      "desc": "恶心、呕吐、然后是一段虚假的好转期。之后的事，看你吃到的剂量。"
    },
    "coPoisoning": {
      "name": "一氧化碳中毒",
      "desc": "头疼、恶心、判断力下降。它没有味道，所以你昨晚睡得很沉。"
    },
    "moldLung": {
      "name": "霉菌性肺病",
      "desc": "咳出来的东西带着颜色。潮湿的地方总要收这笔租金。"
    },
    "kidneyStrain": {
      "name": "肾伤",
      "desc": "腰眼两侧隐隐作痛，尿色深得像茶。回用喝了太久。"
    },
    "despair": {
      "name": "绝望",
      "desc": "你昨天没有理由起床，今天也没找到。所有事情都显得没有意义。"
    }
  },
  "class": {
    "clerk": {
      "name": "便利店店主",
      "title": "你知道哪些货最先卖光",
      "desc": "你在这条街开了九年店。你认识每一个供货司机，也第一个发现瓶装水的进货价涨了三倍。"
    },
    "engineer": {
      "name": "结构工程师",
      "title": "你看得见墙里面的东西",
      "desc": "你算过太多次承重和荷载。别人看到一间屋子，你看到一张受力图——以及它会从哪里先塌。"
    },
    "nurse": {
      "name": "急诊科护士",
      "title": "你见过人是怎么死的",
      "desc": "连续三年夜班。你能在灯光昏暗的走廊里摸到血管，也早就学会了不去记住每一张脸。"
    },
    "veteran": {
      "name": "退役军官",
      "title": "你受过训练，为的正是这一天",
      "desc": "十二年军旅，两次维和。你早就知道秩序是一层很薄的东西，也知道它破掉之后人会变成什么。"
    },
    "hoarder": {
      "name": "囤积者",
      "title": "他们笑了你十年",
      "desc": "你的储物间从地板堆到天花板。家人说你有病，同事说你偏执。你只是比他们更早地不信任这个世界。"
    },
    "hacker": {
      "name": "数据分析师",
      "title": "你能分辨哪条消息是真的",
      "desc": "你的工作是从噪音里找信号。这次要找的东西不在数据库里，在几万条互相矛盾的传闻里。"
    },
    "trucker": {
      "name": "长途货运司机",
      "title": "你有一辆车，和一箱油",
      "desc": "跑了十七年长途。你知道哪条路上没有摄像头，哪个服务区的柴油最便宜，以及一个人在路上要怎么熬过一夜。"
    },
    "chemist": {
      "name": "中学化学老师",
      "title": "你知道什么能中和什么",
      "desc": "你在实验室准备室里待过太多午休。碘、活性炭、次氯酸钠——这些名字对别人是名词，对你是操作步骤。"
    }
  },
  "classPerk": {
    "clerk_network": "熟客网络：采购不受限购影响，且所有采购价 -10%。",
    "engineer_efficiency": "结构直觉：DIY 工时 -25%，且技能不足时不会失败。",
    "nurse_care": "临床经验：治疗效果 +50%，每日患病概率 -25%，自带 1 级医疗站。",
    "veteran_defense": "战术素养：袭击防御 +15%，防守时弹药消耗减半。",
    "hoarder_stash": "十年囤积：起始物资极其丰厚，但行动点 -1、初始理智 -15。",
    "hacker_analysis": "信号识别：情报辨伪率 +20%，自带 1 级电台。",
    "trucker_vehicle": "自带车辆：可去远距离地点，负重上限 +40 kg。",
    "chemist_consumables": "化学储备：滤芯与药品消耗速度 -40%，生水患病率减半。"
  },
  "pack": {
    "none": {
      "name": "什么都没准备",
      "desc": "你和大多数人一样，把这条新闻当成了又一次虚惊。"
    },
    "basic": {
      "name": "一箱水和一箱面",
      "desc": "上周超市促销，你顺手多买了一点。现在这点\"顺手\"值一条命。"
    },
    "medical": {
      "name": "家庭医药储备",
      "desc": "你妈总说家里得备着药。你嫌她啰嗦，但一直没扔。"
    },
    "tools": {
      "name": "装修剩下的材料",
      "desc": "去年装修堆在阳台的木板、螺丝和半桶防水涂料。你一直懒得清。"
    },
    "cash": {
      "name": "取出来的现金",
      "desc": "你在新闻出来的第一天就把定期取了。所有人都说你反应过度。"
    }
  },
  "perk": {
    "perk_camel": {
      "name": "耐渴",
      "desc": "每日饮水需求 -15%。"
    },
    "perk_ironstomach": {
      "name": "铁胃",
      "desc": "喝未净化水的患病概率减半。"
    },
    "perk_scavenger": {
      "name": "拾荒直觉",
      "desc": "搜刮产出 +15%，且能看到地点的剩余存量。"
    },
    "perk_thickblood": {
      "name": "抗寒体质",
      "desc": "舒适线和生存线各降低 4°C。"
    },
    "perk_nightowl": {
      "name": "夜行",
      "desc": "夜间搜刮的危险度 -25%，产出加成提高到 1.6 倍。"
    },
    "perk_ghost": {
      "name": "不存在的人",
      "desc": "遮光帘的每级效果提高 50%，暴露度自然衰减 +3。"
    },
    "perk_builder_hands": {
      "name": "熟练的手",
      "desc": "DIY 工时 -15%。"
    },
    "perk_maintainer": {
      "name": "会保养",
      "desc": "滤芯与发电机机油寿命 +50%。"
    },
    "perk_logistics": {
      "name": "物流关系",
      "desc": "买成品的到货时间 -1 天，被拦截概率显著下降。"
    },
    "perk_electrician": {
      "name": "电工",
      "desc": "发电机产量 +20%，模块耗电 -10%。"
    },
    "perk_salvager": {
      "name": "拆解专家",
      "desc": "拆解回收产出 +40%，且暴露度增量减半。"
    },
    "perk_greenthumb": {
      "name": "会种东西",
      "desc": "菜圃产量 +30%，恶劣环境下的减产减半。"
    },
    "perk_haggler": {
      "name": "会砍价",
      "desc": "所有采购价 -12%。"
    },
    "perk_cashout": {
      "name": "取出来的钱",
      "desc": "开局立刻取出一半银行存款，直接变成手持现金。"
    },
    "perk_analyst": {
      "name": "情报分析",
      "desc": "真情报比例 +12%，且每天可标记一条情报进行核实。"
    },
    "perk_contacts": {
      "name": "人脉",
      "desc": "雇工价格 -20%，成品价格 -12%。"
    },
    "perk_reputation": {
      "name": "街坊眼里的好人",
      "desc": "初始名声 +20，初始社区关系 +20。"
    },
    "perk_leader": {
      "name": "有人愿意跟着你",
      "desc": "同伴士气衰减减半，收留上限 +1。"
    },
    "perk_wellprepared": {
      "name": "早有准备",
      "desc": "起始行动点 +1。"
    }
  },
  "unlock": {
    "site_bunker": "站点：废弃人防工程",
    "site_watertower": "站点：山腰水塔",
    "class_engineer": "职业：结构工程师",
    "class_nurse": "职业：急诊科护士",
    "class_veteran": "职业：退役军官",
    "class_hoarder": "职业：囤积者",
    "class_hacker": "职业：数据分析师",
    "class_trucker": "职业：长途货运司机",
    "class_chemist": "职业：中学化学老师",
    "pack_basic": "物资包：一箱水和一箱面",
    "pack_medical": "物资包：家庭医药储备",
    "pack_tools": "物资包：装修剩下的材料",
    "pack_cash": "物资包：取出来的现金",
    "blueprint_hydroponics": "图纸：立体水培",
    "blueprint_ventilation": "图纸：热回收换气",
    "perk_leader": "天赋：有人愿意跟着你",
    "perk_ghost": "天赋：不存在的人"
  },
  "survivor": {
    "lijie": {
      "name": "李姐",
      "bio": "三楼的邻居，一个人带孩子。会做饭，会缝东西，也会在你不在的时候把门看得很紧。"
    },
    "laochen": {
      "name": "老陈",
      "bio": "对门退休的锅炉工。话少，手很稳，能听出发电机哪个部件在响。",
      "secret": "老陈从床垫下摸出一把猎枪，用油布包着。\"我儿子的。他去年走了。\""
    },
    "xiaoyu": {
      "name": "小雨",
      "bio": "大二学生，学校封了回不去。跑得快，眼睛好，怕黑。"
    },
    "zhaodaifu": {
      "name": "赵医生",
      "bio": "社区医院的全科医生，走的时候只带了一个箱子，里面全是药。",
      "secret": "赵医生终于说了实话：她在医院最后一班接触过确诊病例，一直在自己数天数。"
    },
    "wutou": {
      "name": "吴头",
      "bio": "装修队的工头，带着一整套电动工具。嗓门大，力气大，脾气也大。"
    },
    "linjing": {
      "name": "林警官",
      "bio": "派出所的，制服还穿着，但已经三天没接到指令了。",
      "secret": "林警官说出了那条最后的指令：所有人员就地解散，自行保障。她一直没敢告诉别人。"
    },
    "guming": {
      "name": "顾明",
      "bio": "外卖骑手，全城的路他都熟。有一辆能跑的电动车和一块拆下来的电池。"
    },
    "stranger": {
      "name": "不说名字的人",
      "bio": "你在楼道里遇到他的时候，他正在喝雨水。他没说自己从哪来，你也没问。",
      "secret": "你在他的口袋里发现一张手绘的地图，上面标着这条街每一户人家，你家旁边画了一个圈。"
    }
  },
  "location": {
    "supermarket": {
      "name": "连锁超市",
      "desc": "你每周都来的那家。生鲜区在最里面，水和米面在二号通道。它会是最先被搬空的地方。",
      "descSurvival": "卷帘门歪着。生鲜区的味道已经不是食物。货架被推倒，矿泉水只剩标签。有人在里面过夜。"
    },
    "hardware": {
      "name": "五金建材店",
      "desc": "老板姓王，认得你。木板、螺纹管、防水布、发电机配件——这里的东西没人抢，直到所有人同时想起它们。",
      "descSurvival": "玻璃碎了一地。木板被人拆走了大半，剩下的钉子扎在门口当路障。王老板不在。"
    },
    "pharmacy": {
      "name": "社区药店",
      "desc": "玻璃门上贴着降压药的广告。处方药柜锁着，但柜台后那个年轻店员今天看起来很紧张。",
      "descSurvival": "广告还在，人走了。药柜被撬开又被铁丝重新绕上。碘片的价签掉在地上，字已经看不清。"
    },
    "gasstation": {
      "name": "加油站",
      "desc": "排队的车已经绕了两圈。便利店里的打火机和方便面卖光了，但柴油还有。",
      "descSurvival": "车还在，人已经不排队。油枪垂着，地下罐不知道还剩多少。便利店的门被焊死过一次，又被撬开。"
    },
    "outdoor": {
      "name": "户外用品店",
      "desc": "睡袋、滤水器、冻干食品、煤油炉。价格贵得离谱，但每一样都是为了活下去设计的。",
      "descSurvival": "橱窗空了。还挂着的睡袋被灰盖住。有人在试冲锋衣的口袋里找打火机。"
    },
    "blackmarket": {
      "name": "旧货市场",
      "desc": "铁皮棚子后面那几家，什么都能弄到，只要你不问来路。这里的价格是唯一诚实的东西——它准确反映了世界还剩多少时间。",
      "descSurvival": "棚子还在，货换成了子弹和滤芯。问价的人比卖货的多。有人带着秤，有人带着刀。"
    },
    "bank": {
      "name": "银行网点",
      "desc": "ATM 前排着队，每台都限额。柜台的姑娘说系统很忙，请您理解。现在取出来的每一张纸，三天后可能一文不值，也可能救你一命。",
      "descSurvival": "ATM 黑着，屏幕上还留着最后一次限额的字。大厅玻璃碎了，纸币被人踩进灰里。没有人再排队。"
    },
    "school": {
      "name": "废弃小学",
      "desc": "搬迁了三年，铁门用铁丝拧着。操场的塑胶跑道翻起来了，但食堂的不锈钢水箱还在，教具室里有木板。",
      "descSurvival": "铁丝被人剪开又重新拧上。操场停过车。食堂的水箱盖开着，里面有灰。"
    },
    "hospital": {
      "name": "市第二医院",
      "desc": "急诊门口的地上还有担架轮子压出的痕迹。药房在地下一层，但走廊里的味道会告诉你这里发生过什么。",
      "descSurvival": "担架轮子的印子被灰盖住。药房的门开着，货架空了。走廊尽头有人用过火。"
    },
    "warehouse": {
      "name": "城郊仓储中心",
      "desc": "电商的区域仓，卷帘门有二十扇。如果还没被清空，这里的存量足够一个家庭活过整个冬天。前提是你能把东西运回来。",
      "descSurvival": "二十扇门有几扇开着。纸箱被撕开，泡沫满地。里面可能还有货，也可能有人等你先走进去。"
    },
    "servicearea": {
      "name": "高速服务区",
      "desc": "出城四十公里。加油站的地下储罐里通常还剩几百升——如果你有泵，有软管，有胆子。",
      "descSurvival": "服务区的灯早灭了。地下罐的井盖被人撬过。路上有车壳，没有人给你加油。"
    }
  }
};

registerTree('world', data);
