import { registerTree } from '../../t';

export const data = {
  "pressure_passerby": {
    "beggar": {
      "title": "有人敲门，只敲了三下",
      "body": "敲门声很轻，像是怕吵到别人。门外站着一个人，很瘦，外套还算干净。\n\"我不进来，\"他说，\"能给点水吗？半瓶就行。\"",
      "choice": {
        "give": {
          "label": "递出去一瓶水",
          "log": "你从门缝里递出去一瓶水。他说了谢谢，走了。现在有人知道这里有水。"
        },
        "silent": {
          "label": "不出声，等他走",
          "log": "你贴着门站了十分钟，直到脚步声下了楼。你的腿有点麻。"
        },
        "info": {
          "label": "告诉他哪里有积水可以取",
          "log": "你隔着门告诉他小学操场的储水箱还有水。他谢了你。"
        }
      }
    },
    "distant_figure": {
      "title": "路上停了一辆车，很久没走",
      "body": "车停在进来的那条土路上，离这里大概八百米，一直没有人下来。\n一个小时后它才掉头离开，你在望远镜里看见副驾驶座上有人始终朝这个方向看。",
      "choice": {
        "hide_tracks": {
          "label": "把院子里的痕迹和烟囱的烟都处理掉",
          "log": "你把车辙扫平，把烟囱改成了贴地排烟。这个地方从路上看起来又空了。"
        },
        "watch": {
          "label": "什么都不做，继续观察",
          "log": "你记下了车牌的后三位。第二天同一时间，它又来了。"
        }
      }
    },
    "vent_voice": {
      "title": "通风井里传来说话声",
      "body": "上面说话的声音不是冲你来的。两个人在聊天，声音顺着管道下来，听得很清楚。\n\"这下面以前是停车场吧。\"\n\"锁着呢。\"\n\"锁着不代表没人。\"",
      "choice": {
        "silent": {
          "label": "关掉一切声源，屏住呼吸",
          "log": "你把所有电器都断了，在黑暗里坐了四十分钟。上面的人走了。"
        },
        "noise": {
          "label": "故意制造塌方的动静，让他们以为这里危险",
          "ok": {
            "log": "你在远处的通道推倒了一堆水泥板。上面的人骂了一句，再没回来过。"
          },
          "bad": {
            "log": "声音太真了，反而证明了下面有人。他们在井口待了很久。"
          }
        }
      }
    }
  },
  "pressure_scout": {
    "window_watcher": {
      "title": "有人在盯着这里",
      "body": "你是在倒水的时候看见他的。不远处站着一个人，已经在那里至少站了二十分钟，一直朝这个方向看，不是路过。\n你昨晚开了一整夜发电机。",
      "choice": {
        "blackout": {
          "label": "今晚全黑，一点声音都不出",
          "log": "你切断了所有电源。一整晚，一家人在黑暗里坐着，谁也没说话。"
        },
        "show_force": {
          "label": "让他看见你有枪",
          "reason": "需要有弹药",
          "log": "你把猎枪举到他能看见的地方，他退了两步，转身走了。这里有枪这件事会从他嘴里传出去，后果是好是坏还说不准。"
        },
        "confront": {
          "label": "直接出去问他要干什么",
          "ok": {
            "log": "他说他在找妹妹，从周三就开始找了。你给了他一点水。他记住了你的门牌号，但这次是好的那种记住。"
          },
          "bad": {
            "log": "他什么也没说，只是看了你一阵，然后慢慢走开了。两天后你会知道他去找了什么人。"
          }
        },
        "ignore": {
          "label": "装作没看见",
          "log": "你放下了窗帘，他还站在原地没走。"
        }
      }
    },
    "ramp_mark": {
      "title": "坡道口的墙上多了一个记号",
      "body": "闸门左侧的柱子上多了一个粉笔画的三角形，边长大概二十厘米，你昨天下班回来时它还不在。\n这个符号你在别的地方见过，上一次见到它，是在一栋后来被搬空的楼门口。",
      "choice": {
        "erase": {
          "label": "擦掉它，再画一个别的",
          "log": "你擦掉了三角形，在旁边画了一个圆圈加一道斜线。你不知道这代表什么，但看起来像\"已清\"。"
        },
        "watch_ramp": {
          "label": "在坡道上布置警报（3 零件）",
          "log": "你在坡道拐弯处拉了一条细铁丝，另一端连着一串罐头盒，有人碰到它，你在屋里就能听见。"
        },
        "ignore": {
          "label": "一个粉笔记号而已",
          "log": "你没管它，三天后你会从别人嘴里知道那个三角形是什么意思。"
        }
      }
    },
    "tire_tracks": {
      "title": "雪地上多了一组脚印",
      "body": "早上出门倒灰的时候，你看见雪地上多了一组脚印，从院墙外一直走到窗下，在那里站了一会儿又原路回去。\n脚印的鞋码不小，而且对方清楚该往哪扇窗户看。",
      "choice": {
        "cover": {
          "label": "把所有痕迹扫掉，加强伪装",
          "log": "你花了一上午把院子里所有的痕迹清掉，又在门口堆了些看起来很久没人动过的杂物。"
        },
        "trap": {
          "label": "在窗下布置警报（消耗零件）",
          "log": "你用铁丝、罐头盒和几个螺母做了一套土警报。它不能挡人，但能让你提前三十秒知道。"
        },
        "wait": {
          "label": "什么也不做，等着看",
          "log": "你决定按兵不动，第二天脚印又多了一组。"
        }
      }
    }
  },
  "pressure_tribute": {
    "street_toll": {
      "title": "街口有人设了卡，进出要交东西",
      "body": "两个油桶架着一根钢管，旁边生着火。四五个人轮流守在那儿，谁都不穿制服。\n他们没来敲你的门，只是让你知道：从今天起，这条街进出要\"打招呼\"。\n傍晚有人来传话，说明天开始每户要交东西，不多，\"够我们守夜就行\"。",
      "choice": {
        "pay": {
          "label": "交（1 份口粮 + 4 L 水）",
          "log": "你交了东西。守杆的人给了你一个用马克笔画的记号，让你贴在门上。"
        },
        "join": {
          "label": "不交钱，去轮守夜",
          "log": "你去守了半夜。冷得要命，但你认识了这条街上还活着的每一个人。"
        },
        "refuse": {
          "label": "不理这套",
          "log": "你没交，也没去。第二天你的门上被人用马克笔画了一个不一样的记号。"
        }
      }
    },
    "militia_tax": {
      "title": "北巷自治队上门收保护费",
      "body": "四个人，两个带着猎枪，袖子上缠着一圈红布。领头的很客气，甚至没有敲门，只是在门外说话。\n\"我们是北巷自治队。这条街现在由我们维持秩序。每周两份口粮、五升水，我们保证没人来动你。\"\n\"你可以不交。但那样我们就不知道谁会来。\"",
      "choice": {
        "pay": {
          "label": "交（2 份口粮 + 5 L 水）",
          "log": "你交了保护费。领头的在一个小本子上记了你的门牌号，说下周同一时间。"
        },
        "negotiate": {
          "label": "谈条件：用手艺抵",
          "ok": {
            "log": "你说你会修发电机。领头的想了想，说那你每周来修两台，粮就免了。"
          },
          "bad": {
            "log": "他听完笑了，然后拿走了比原来更多的东西。\"下次别废话。\""
          }
        },
        "refuse": {
          "label": "拒绝",
          "log": "你说不交。领头的点点头，说理解，然后在本子上画了个别的记号。"
        }
      }
    },
    "gov_requisition": {
      "title": "巡逻队要\"清点\"你的物资",
      "body": "两辆车，六个人，制服是真的，臂章也是真的。他们带着表格。\n\"战时物资统一调配。我们清点，登记，开条。等秩序恢复后凭条领回补偿。\"\n领头的军官很年轻，看起来自己也不太相信那张条子。他身后的人已经开始往屋里走。",
      "choice": {
        "comply": {
          "label": "配合，让他们登记",
          "log": "他们拿走了六份口粮、十四升水和六升油，留给你一张盖了红章的白条。你把它压在了台历下面。"
        },
        "hide": {
          "label": "提前藏起大半，只交出一部分",
          "ok": {
            "log": "你把主要的存货移到了通风管道后面。他们清点了摆在外面的那些，签字走了。"
          },
          "bad": {
            "log": "那个年轻军官盯着地板上被拖过的痕迹看了三秒，然后指了指管道。他们把藏的全拿走了，还有你的药。"
          }
        },
        "militia_card": {
          "label": "说这条街由自治队管着",
          "reason": "需要已在向自治队交保护费",
          "log": "你说出了自治队的名字。军官记下来了。你保住了物资，但从今天起你在他们的名单上换了一栏。"
        }
      }
    },
    "boat_toll": {
      "title": "划艇的人在窗外收\"过路费\"",
      "body": "水已经到了一楼窗台以上。三个人一条充气艇，绕着这片楼慢慢划。\n他们不上来，只是在下面喊：\"想出去就说话，一趟两份罐头。不出去也行，我们每天都来。\"\n他们知道你在，也知道这片楼里每一户人都还在。",
      "choice": {
        "pay": {
          "label": "花 2 份罐头换一趟出行",
          "log": "你付了两份罐头。他们说明天上午会来接你，去哪都行，只要不太远。"
        },
        "refuse": {
          "label": "不理，你能等水退",
          "log": "你没搭话。艇又绕了两圈才走。领头的那个抬头看了很久你所在的那扇窗。"
        },
        "radio": {
          "label": "用无线电联系救援队，绕过他们",
          "reason": "需要 1 级无线电",
          "log": "你在 121.5 上呼了三十分钟，终于有人应答。他们记下了你的坐标和人数。"
        }
      }
    }
  },
  "raid_attempt": {
    "crowbar": {
      "title": "有人在踹门",
      "body": "门外不是敲，是踹。一共三个人，其中一个手里有撬棍，另外两个替他挡着楼道口。\n他们不说话也不喊话，说明这种事他们做过很多次了。\n门框已经在响。",
      "choice": {
        "barricade": {
          "label": "用一切能搬的东西堵住门",
          "log": "你把冰箱、书架和装满水的桶全推到了门后，门板后面的重量让踹门声闷了下去。"
        },
        "shoot": {
          "label": "朝门外开一枪",
          "reason": "需要 1 发弹药",
          "log": "你朝门板上方开了一枪。楼道里瞬间安静，然后是很快的脚步声。枪声会传得很远。"
        },
        "talk": {
          "label": "隔着门跟他们谈",
          "ok": {
            "log": "你从门缝递出三份罐头，说楼上还有两户空房可以撬。他们收了，走了。"
          },
          "bad": {
            "log": "门外那个人只回了一句：\"开门。\"然后继续踹。"
          }
        },
        "hide": {
          "label": "带上能拿的躲进里屋",
          "log": "你抓了药和一半的罐头躲进卫生间，反锁上门，背贴着门坐在地上。"
        }
      }
    },
    "requisition_raid": {
      "title": "上次来登记的人又来了，没带本子",
      "body": "来的人还是那身制服，但少了两个人，也没有车，臂章还缝在袖子上，针脚已经歪了。\n\"最后一次登记。\"领头的说，他的枪没有再背在肩上。\n这次没人带本子，也不开白条，这帮人已经不像上次来登记的政府军。",
      "choice": {
        "give_all": {
          "label": "全给他们，保命",
          "log": "你什么都给了。领头的临走时说了句\"对不住\"，声音很低。"
        },
        "fight": {
          "label": "抵抗",
          "reason": "需要有弹药",
          "log": "你决定不给，隔着门僵持了大约三十秒，外面的脚步声就走远了。"
        },
        "appeal": {
          "label": "喊出那个年轻军官的名字",
          "reason": "需要之前配合过登记",
          "ok": {
            "log": "你叫了他的姓。他愣了一下，只让人拿走了一小部分，还留下两盒压缩饼干。"
          },
          "bad": {
            "log": "他没回头。拿走的东西一点没少。你不确定还是不是上次那个人。"
          }
        }
      }
    },
    "boat_raid": {
      "title": "有人在撬你二楼的窗户",
      "body": "水面离窗台只有半米。一条橡皮艇拴在防盗网上，两个人踩在艇沿，正用一根撬棍撬窗框。\n他们湿透了，看起来已经这么干了好几家。\n你脚下的一楼已经全是水，你无处可退，他们也一样。",
      "choice": {
        "push_boat": {
          "label": "推开他们的艇",
          "ok": {
            "log": "你抓住撬棍猛地一带，艇失了平衡。他们没落水，但骂着划开了。"
          },
          "bad": {
            "log": "你抓住了撬棍，然后被它砸中了小臂。窗户开了一半。"
          }
        },
        "trade": {
          "label": "主动给一点，换他们走",
          "log": "你从窗缝塞出两份罐头和几瓶水。他们接了，解开绳子划走了。撬棍留在了窗台上。"
        },
        "defend": {
          "label": "守住窗户",
          "log": "你把桌子横着顶在窗框上，用整个身体压住，撬棍每撬一下桌子就跟着震一下。"
        }
      }
    },
    "vent_gasoline": {
      "title": "井上的人喊话：不开门就倒汽油",
      "body": "声音顺着管道传下来，带着回声，听起来像是从四面八方来的。\n\"我们知道下面有人，能听见机器的声音。\"\n停了几秒，对方接着说：\"开门，还是我们往里面倒汽油，你自己选。\"\n你抬头看了一眼格栅，它离地有四米。",
      "choice": {
        "seal_vent": {
          "label": "立刻封住通风井（消耗建材）",
          "log": "你用木板和湿布把井口从下面堵死了。汽油没进来，但接下来的六小时你们几乎无法呼吸。"
        },
        "open": {
          "label": "开门，把东西给他们",
          "log": "你打开了闸门。他们进来了，拿了很多。走的时候有人拍了拍你的肩膀。"
        },
        "bluff": {
          "label": "喊回去：这下面是消防水泵房，有人值守",
          "ok": {
            "log": "你用最平静的声音报出了一串编号和一个不存在的值班表。上面沉默了很久，然后走了。"
          },
          "bad": {
            "log": "上面那个人笑了。\"水泵房没有柴油机的声音。\"然后你听见了塑料桶的响声。"
          }
        }
      }
    }
  },
  "frozen_crowd": {
    "main": {
      "title": "门口有一群冻僵的人",
      "body": "七八个，也许更多。他们没有撬棍，也没有喊话，只是挤在门廊下背着风。\n最外面那个已经不动了。有个女人抱着一个用棉被裹起来的东西，看不出是不是活的。\n他们是来躲冷的，你屋里有暖气，就没打算再走。",
      "choice": {
        "let_in": {
          "label": "开门，让他们进来取暖",
          "log": "你开了门。屋里挤了十几个人，一整晚都有人在哭。天亮之后他们陆续走了，灶膛里的柴烧掉了大半。"
        },
        "let_child": {
          "label": "只让那个抱孩子的女人进来",
          "log": "你只开了一条缝，把她拉了进来，然后立刻关上。门外的人没有骂你，这更难受。"
        },
        "closed": {
          "label": "不开门",
          "log": "你没开门，在屋里一直听着，直到外面再没有一点声音。"
        }
      }
    }
  },
  "raid_aided_repel": {
    "shelter_crowd": {
      "title": "要破门的人被楼道里的人堵住了",
      "body": "猫眼里进来三张脸，为首的那个手里攥着撬棍，另外两个把楼道口堵得严严实实。你贴着猫眼不敢出声，手心里全是汗。\n他们刚要抬手砸门，楼梯拐角传来一阵脚步声，七八个人从黑暗里围了上去。你听见有人叫了你的门牌号，接着是推搡声和几句压得很低的狠话。\n撬棍最后没有落到你的门上。那伙人对峙了一阵，退下楼去，脚步声乱成一团。",
      "choice": {
        "help": {
          "label": "开门出去，和他们站在一起",
          "log": "你拉开门加入了对峙。人多势壮，那伙人扔下几句狠话就散了，收留过的人里有一个懂木工，顺手把你被撞松的门框敲了回去。"
        },
        "watch": {
          "label": "守在门后，别出声",
          "log": "你贴着门听完整个对峙，直到楼下彻底安静才敢坐回地上。他们走之前敲了敲你的门，告诉你今晚没事了。"
        }
      }
    }
  },
  "env_ash_roof": {
    "roof_load": {
      "title": "屋顶的积灰已经压出了裂缝",
      "body": "屋顶的火山灰比雪重得多，被打湿之后还要更重，你听见天花板某个位置发出了一声很轻的\"咔\"。\n再不清它就会塌，可清灰意味着你要在外面待上两个小时，吸进很多东西。",
      "choice": {
        "clear": {
          "label": "上去清灰",
          "log": "你在屋顶铲了两个小时，口罩很快糊住，只能换用围巾顶着。"
        },
        "clear_masked": {
          "label": "戴好防护再上去",
          "reason": "需要 1 级空气过滤提供的防护装备",
          "log": "你戴上正压面罩上了屋顶。灰还是灰，但至少没进肺里。"
        },
        "ignore": {
          "label": "不管，赌它撑得住",
          "log": "凌晨三点，厨房那一片天花板塌了。灰和水泥块埋了半个灶台。"
        }
      }
    }
  },
  "env_flood_rise": {
    "underground_flooding": {
      "title": "水从坡道流进来了",
      "body": "你先听见的是那种持续又很宽的哗哗声，接着闻到了水的腥气。\n负一层已经灌满了，水正沿着车道往下淌，五分钟前还在门槛外面，现在已经没过你的脚踝。\n这里是负二层，没有更低的地方，也没有向上的路。",
      "choice": {
        "evacuate": {
          "label": "立刻放弃这里，能搬多少搬多少",
          "log": "你在水涨到腰之前搬了三趟，剩下的都留在了下面。你花了三十天建起来的东西，四十分钟就没了。"
        },
        "dam": {
          "label": "在坡道上筑挡水墙（消耗大量建材）",
          "ok": {
            "log": "你用沙袋、木板和一整桶水泥封住了坡道。水停在了外面，只渗进来一点。"
          },
          "bad": {
            "log": "墙没扛住。半夜水推倒了它，你在齐胸深的冷水里抢出了几箱东西。"
          }
        },
        "pump": {
          "label": "用水泵往外抽（需要发电）",
          "reason": "需要发电与 4 零件",
          "log": "你接了一台潜水泵，往坡道外抽。它的声音很大，一整夜都在响。水位停住了。"
        }
      }
    },
    "water_contaminated": {
      "title": "储水里有味道",
      "body": "打开水桶的时候你闻到了一股味道，不明显，是一点土腥味混着别的东西。\n洪水已经把全城的下水道和化粪池搅在了一起，而你的储水桶昨天刚开过盖。",
      "choice": {
        "boil": {
          "label": "全部烧开（费燃料）",
          "log": "你把所有水分批烧开。烧开能杀死细菌，但杀不死化学物质。你只能希望是前者。"
        },
        "filter": {
          "label": "过一遍净水系统",
          "reason": "需要 1 级净水",
          "log": "你把水全部倒回原水桶，让净水器重新跑了一遍。这就是你当初装它的理由。"
        },
        "drink": {
          "label": "就这么喝，你没有别的水",
          "log": "你把它们喝了下去，第二天早上你就知道自己错了。"
        }
      }
    }
  },
  "env_cold_snap": {
    "pipes": {
      "title": "水管冻了",
      "body": "早上拧开龙头没有水，接着管道在墙里发出一声闷响。\n如果管子已经冻裂，解冻的时候你会失去墙里的一切；现在唯一能确定的是，今天不会有自来水了。",
      "choice": {
        "thaw": {
          "label": "用热水和布慢慢解冻",
          "log": "你花了一上午用热水和布一段一段地敷，管子通了，也没有裂。"
        },
        "torch": {
          "label": "直接用火烤快一点",
          "ok": {
            "log": "你控制着火焰只烤接头的地方，十分钟就通了。"
          },
          "bad": {
            "log": "接头爆了。墙里喷了两分钟，你的存水少了十二升，墙皮全湿了。"
          }
        },
        "wait": {
          "label": "不管它，反正也不指望自来水",
          "log": "你把水管的事放下了，反正你早就在靠储水过日子。"
        }
      }
    }
  },
  "med_neighbor_sick": {
    "fever_at_door": {
      "title": "李姐来敲门，孩子烧到四十度",
      "body": "是李姐。她站得离门有两米远，戴着口罩，声音是哑的。\n\"孩子烧到四十度了。我知道我不该来。\"她说，\"你有药的话，放在楼梯口就行，我等你上去了再拿。\"\n她说完就往后退了两步，退到猫眼看不见她的位置。",
      "choice": {
        "give_meds": {
          "label": "把药放在楼梯口",
          "log": "你把三组药放在楼梯口后退回屋里，听见她上楼取走药，隔着楼道说了声谢谢。"
        },
        "go_treat": {
          "label": "亲自过去给孩子处置",
          "reason": "需要医疗 3 级",
          "log": "你去了。孩子的呼吸声不对，你处理了三个小时。回来之后你洗了很久的手。"
        },
        "refuse": {
          "label": "隔着门说没有",
          "log": "你说没有，她道了声谢就下楼了。你听着她的脚步声一层一层地远了。"
        }
      }
    }
  },
  "med_own_wound": {
    "red_line": {
      "title": "那条红线又往上走了两厘米",
      "body": "早上量的时候红线还在手腕上方三指，现在已经快到肘窝了。\n伤口本身已经不疼了，这不是好消息，你知道再往上去的是什么。\n你手里有药，但不多，也可以选更狠的办法。",
      "choice": {
        "meds": {
          "label": "按疗程用药",
          "log": "你按剂量吃了整个疗程。第三天早上，那条线开始往回退。"
        },
        "debride": {
          "label": "自己清创",
          "reason": "需要 1 级医疗站",
          "ok": {
            "log": "你咬着毛巾，用半瓶碘伏把坏死的部分刮掉了，红肿第二天就开始退。"
          },
          "bad": {
            "log": "你下手了，但没清干净，还弄破了别的地方。第二天你几乎起不来。"
          }
        },
        "wait": {
          "label": "省药，扛过去",
          "log": "你决定省着药扛过去，你告诉自己身体会赢，虽然没什么把握。"
        }
      }
    }
  },
  "opp_trader": {
    "cart_man": {
      "title": "有个推车的人在敲每一家的门",
      "body": "一辆改装的手推车，上面用绑带固定着三个箱子。他很瘦，但眼睛很清醒。\n\"不要钱，\"他说，\"钱现在是纸。我要药，要电池，要能修东西的零件。\"\n他掀开一个箱子：里面是罐头、盐、火柴，和一小袋种子。",
      "choice": {
        "trade_meds": {
          "label": "用 2 组药换食物和种子",
          "log": "你用两组药换了七份罐头和一小袋菜种。他说下次还来。"
        },
        "trade_parts": {
          "label": "用 5 零件换燃料",
          "log": "你用五个零件换了十升柴油。成交的时候他笑了，说你懂行。"
        },
        "buy_intel": {
          "label": "花 1 组药买情报",
          "log": "他压低声音说了三个地方：哪个仓库还没被清、哪条路上有人设卡、以及北边确实在收人。你把坐标写进日记。",
          "blocked": [
            "路上有人设卡"
          ]
        },
        "pass": {
          "label": "你现在没有他要的东西",
          "log": "你摇了摇头。他说没关系，问你需不需要水——他给了你两瓶，说下次记得就行。"
        },
        "rob": {
          "label": "他一个人，车上有你要的东西",
          "reason": "需要有弹药",
          "log": "你抢了他，他没有反抗，只是一直看着你直到你放他走。他不会走远，这件事他会告诉所有人。"
        }
      }
    }
  },
  "opp_trader_warehouse": {
    "main": {
      "title": "商人说的那个仓，卷帘门还虚掩着",
      "body": "你按日记上的坐标走到仓储中心外围，那里没有灯，卷帘门的锁被撬过，又被人用铁丝随便绕上。\n设卡的那条路你绕开了，北边的事还写在日记的同一页。",
      "choice": {
        "note": {
          "label": "确认还能进去，改天带包来",
          "log": "你没有今晚就冲进去，只从门缝里闻到了纸箱的味道，确认存量还在，打算改天带包再来。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你看了一眼就走了，坐标还留在日记里。"
        }
      }
    }
  },
  "opp_supply_drop": {
    "ration_point": {
      "title": "广播里报了一个配给点的地址",
      "body": "每天上午九点到十一点，凭身份证领取，每人限一份。地址在三公里外的体育馆。\n广播把地址念了三遍，然后是国歌。\n配给点在三公里外，来回要两个小时，而且那里会聚起很多人，这两件事都得掂量。",
      "choice": {
        "go": {
          "label": "去排队",
          "log": "你排了两个小时，领到一箱压缩食品和两桶水。队伍里有人在咳嗽。"
        },
        "go_with_crew": {
          "label": "带同伴一起去，能多领几份",
          "reason": "需要有同伴",
          "log": "你们一起去，领了几份回来。回来的路上有人跟了你们两个街口。"
        },
        "pass": {
          "label": "不去，人多的地方比饿更危险",
          "log": "你没去。下午广播说配给点发生了拥挤事件，暂停发放。"
        }
      }
    }
  },
  "story_neighbor_outcome": {
    "child_lived": {
      "title": "楼梯口多了个铁盒，孩子好了",
      "body": "里面是一把钥匙、一张手绘的管道图，和一张纸条。\n\"孩子好了。地下室水泵房的钥匙，里面有个储水口。图画得不好，照着能找到。\"\n最后一行只有四个字：\"欠你一次。\"",
      "choice": {
        "take": {
          "label": "收下",
          "log": "你去了水泵房。消防水箱是满的，两百多升。你把钥匙揣进兜里，往回走的时候走得很慢。"
        }
      }
    },
    "child_died": {
      "title": "三楼的哭声停了，药退了回来",
      "body": "你昨天听见的哭声在半夜停了。\n今天早上门口放着一个塑料袋，里面是你给的那几板药，只用了两片，没有留下纸条。",
      "choice": {
        "accept": {
          "label": "把药收回来",
          "log": "你把药收了回来。它们会救别人的命，这个想法没让你好受一点。"
        },
        "leave": {
          "label": "不动它",
          "log": "你没去拿。那个袋子在门口放了三天，然后不见了。"
        }
      }
    }
  },
  "story_frozen_morning": {
    "morning_after": {
      "title": "早上门推不开",
      "body": "门被什么东西从外面抵住了，你用了很大力气才推开一条缝。\n他们大部分已经走了，剩下两个还留在门廊里，姿势像是睡着了，棉被裹着的那个东西还在其中一个怀里。\n雪已经把他们盖了薄薄一层。",
      "choice": {
        "bury": {
          "label": "把他们搬到楼后，做个标记",
          "log": "你花了一上午。你不知道他们叫什么，就在墙上用炭写了\"两个人，一个孩子\"和日期。"
        },
        "take": {
          "label": "他们身上有些东西还能用",
          "log": "你从他们的背包里拿了些东西。你的手抖得很厉害，但你还是拿了。"
        },
        "close": {
          "label": "关上门，从后窗走",
          "log": "你把门重新关上，之后一个星期都走后窗。"
        }
      }
    }
  },
  "story_family_radio": {
    "county_broadcast": {
      "title": "无线电里念到了那个县城的名字",
      "body": "一段民用频段的转播，信号很差。念的是安置点名单，一个县一个县地念。\n念到那个名字的时候，后面跟着的是一串数字：接收人数、失联人数。\n第二个数字比第一个大。",
      "choice": {
        "call": {
          "label": "试着呼叫那个频段，问名单",
          "reason": "需要 2 级无线电才能发射",
          "ok": {
            "log": "有人回了你，他把名单翻了三遍，最后报出一个床位号，说人在，她还在。"
          },
          "bad": {
            "log": "没有人应答，你呼叫了两个小时，最后只剩下自己的回声。"
          }
        },
        "listen": {
          "label": "只是听着，记下数字",
          "log": "你把两个数字记在了本子上，然后关掉了收音机。"
        }
      }
    },
    "stair_rumor": {
      "title": "楼道里有人在传安置点的名单",
      "body": "消息没经收音机也传到了六楼，纸是手抄的，县名写错了一个字，但你还是认了出来。\n县名后面跟着两个数字，第二个比第一个大。",
      "choice": {
        "ask": {
          "label": "追问抄纸的人",
          "log": "对方说名单是别人抄来的，数字对不上号，你顺手记下了那个写错的字。"
        },
        "keep": {
          "label": "把纸带回家",
          "log": "你把纸带回家摆在桌上，那两个数字你看了很多遍。"
        }
      }
    }
  },
  "story_convoy_news": {
    "convoy_fate": {
      "title": "车队的消息传回来了",
      "body": "是从旧货市场那边听来的，转了三四手。\n出城的国道在第二个收费站被堵了，十一辆车里有六辆调头，剩下五辆走了小路。\n之后的部分没人说得清。只有人提到，其中一辆车后来出现在了另一批人手里。",
      "choice": {
        "reflect": {
          "label": "你留下了",
          "log": "你想起自己交的那三千块，也想起自己没上那辆车。报名那十一户，你现在能报出名字的不到五个。"
        }
      }
    }
  },
  "story_gov_ration": {
    "the_receipt": {
      "title": "那张白条派上用场了",
      "body": "一辆卡车停在街口，还是那身制服，这次是凭之前的登记条发东西。\n队伍不长，大部分人当时把物资藏了起来，现在手里没有条。\n你把压在台历下面的那张纸拿了出来。",
      "choice": {
        "claim": {
          "label": "去领",
          "log": "他们按条子上的数量给了你，还多给了两组药。发药的人没看你，只低头对了条子上的号码。"
        },
        "share": {
          "label": "领了，分一部分给没有条的邻居",
          "log": "你把一半分给了楼里没登记的几家。送最后一家的时候，对方隔着门缝看了你很久，什么也没说。"
        }
      }
    }
  },
  "dream_sequence": {
    "supermarket_dream": {
      "title": "你又回到了那家超市",
      "body": "灯全亮着，冷柜在响，货架是满的，广播里在念一个促销活动。\n你推着车走过二号通道，弯下腰拿了最后一箱矿泉水。\n直起身的时候，收银台后面站着的是李姐，她的口罩上有血，正在等你结账。\n\"你少拿了一样东西。\"她说。",
      "choice": {
        "wake": {
          "label": "醒来",
          "log": "你在凌晨四点醒了，一身汗。被窝里已经凉了。你坐着等心跳慢下来，才又躺下。"
        },
        "ask": {
          "label": "问她少拿了什么",
          "log": "她没有回答，只是指了指你身后，你一转身梦就结束了。醒来的时候，你觉得心里轻了一点。"
        }
      }
    }
  }
};

registerTree('event', data);
