import { registerTree } from '../../t';

export const data = {
  "stat_arc_sanity_1": {
    "main": {
      "title": "水管里有人用你的声音说话",
      "body": "你把耳朵贴上去。它说了一句你早上刚说过的话，然后笑了一下。\n窗外的灰一下一下打在窗台上，落点很匀。",
      "choice": {
        "listen": {
          "label": "再听一句",
          "log": "第二句不是你的声音。你把耳朵拿开。"
        },
        "tap": {
          "label": "敲两下让它停",
          "log": "它停了。你不确定是吓走了，还是换了地方。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你把水龙头开到最大，用流水盖过它。"
        }
      }
    }
  },
  "stat_arc_sanity_2": {
    "main": {
      "title": "落灰一声一声，你听成了脚步",
      "body": "灰打在窗台上，一下一下。你听成有人在上一层轻轻走。你数到十七停了——十七是你门牌。",
      "choice": {
        "write": {
          "label": "写在纸上：落的是灰",
          "log": "你写了：落的是灰。把纸条贴在门上，免得夜里又听错。"
        },
        "wait": {
          "label": "等到真有脚步再反应",
          "log": "你等了一夜。没有人。灰还在落。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你把枕头压在耳朵上。"
        }
      }
    }
  },
  "stat_arc_sanity_break": {
    "main": {
      "title": "日历自己往回翻",
      "body": "你看见准备期的自己在超市排队。她回头看你，嘴巴动了，说的是今天的日期。\n药品柜里的碘片包装在反光。你不记得有没有买。",
      "choice": {
        "sit": {
          "label": "坐到数字不再跳",
          "log": "你坐着。墙上的日历你没再翻。"
        },
        "eat": {
          "label": "强迫自己吃一口",
          "log": "嘴里有味道之后，你把日历翻回今天。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你让它翻。日期你自己记得。"
        }
      }
    }
  },
  "stat_arc_hp_1": {
    "main": {
      "title": "上楼走到一半，腿软了，你坐下来",
      "body": "你本想一口气走完。腿先软了，人跟着坐在台阶上。心跳比平时响。\n药箱还在屋里原来的位置。",
      "choice": {
        "slow": {
          "label": "承认今天只能慢着走",
          "log": "你把步子改小。小步也能到家。"
        },
        "ignore": {
          "label": "站起来继续走",
          "log": "你走完了。膝盖在疼，你扶着墙下楼。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你在楼梯上坐到腿麻。"
        }
      }
    }
  },
  "stat_arc_hp_2": {
    "main": {
      "title": "伤口不疼了，你更慌",
      "body": "疼的时候你至少知道那块肉还连着。现在按下去像按别人的胳膊，没什么感觉。你还是按了几下。药箱就在旁边。",
      "choice": {
        "clean": {
          "label": "清洗、包扎、少出门",
          "log": "纱布是干净的。你还能管这具身体。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你把袖子放下来。看不见就不去想。"
        }
      }
    }
  },
  "stat_arc_hp_crit": {
    "main": {
      "title": "你开始给自己留纸条：如果倒了",
      "body": "字写得很慢。你写了水在哪、钥匙在哪、不要开窗。写完你把纸条夹进日记夹层，字朝里。",
      "choice": {
        "keep": {
          "label": "把纸条夹进日记",
          "log": "日记变厚了一页。你希望用不到。"
        },
        "tear": {
          "label": "撕掉，当它没写过",
          "log": "你撕了。纸屑掉在脚边。字没了，内容你还记得。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你盯着那张纸，直到天又暗了。"
        }
      }
    }
  },
  "stat_arc_stamina_1": {
    "main": {
      "title": "背包刚拎起来，肩就软了",
      "body": "包没比昨天重，是你没力气了。你把包放下，又拎起来，又放下。光是这样就已经喘。",
      "choice": {
        "half": {
          "label": "减负，只带必要的",
          "log": "包轻了。门口的风还是那么大。"
        },
        "push": {
          "label": "咬牙出门",
          "log": "你出了门。第三层楼梯你又坐下来。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "今天不出。包在门口等。"
        }
      }
    }
  },
  "stat_arc_stamina_2": {
    "main": {
      "title": "你在外面中途坐倒，有人从旁边走过去",
      "body": "他们看了你一眼。没有停。你理解。你要是有力气，也不会停。",
      "choice": {
        "wave": {
          "label": "抬手表示你还活着",
          "log": "有人点了下头。然后走了。"
        },
        "hide": {
          "label": "把脸埋进膝盖",
          "log": "脚步远了。你的脸很烫。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你等到自己能站起来。没有人回头。"
        }
      }
    }
  },
  "stat_arc_humanity_1": {
    "main": {
      "title": "楼道里有人看见你，把孩子转到自己身后",
      "body": "动作很小。你还是看见了。他们把孩子往身后揽，加快脚步上楼。",
      "choice": {
        "nod": {
          "label": "点头，表示你懂",
          "log": "你点头。他们把孩子护得更紧，脚步加快了。"
        },
        "speak": {
          "label": "说一句\"我没有恶意\"",
          "log": "你的声音在楼梯间发空。没有人回答。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你低头继续走。那个转身你没接。"
        }
      }
    }
  },
  "stat_arc_humanity_2": {
    "main": {
      "title": "有人来求助，你一秒就算完了：不帮",
      "body": "以前你会为算得这么快觉得丢脸。现在你数完手头的水、药、门能挡多久，答案还是不。",
      "choice": {
        "hate": {
          "label": "把这个速度当成病",
          "log": "你讨厌自己算得这么快。还是走开了。"
        },
        "keep": {
          "label": "承认这就是新规矩",
          "log": "你没有改答案。一秒够了，你走开了。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你去洗手。水龙头开了一会儿。"
        }
      }
    }
  },
  "stat_arc_rep_1": {
    "main": {
      "title": "你敲门，里面有脚步，没有人开",
      "body": "脚步停在门后。呼吸很近。然后退远。你认识这扇门，以前借过盐。",
      "choice": {
        "leave": {
          "label": "留下半瓶水就走",
          "log": "瓶子在门口。你下楼的时候没有回头。"
        },
        "wait": {
          "label": "再敲一次",
          "log": "你敲了。脚步更远。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你离开。门后的人松了一口气，你听见了。"
        }
      }
    }
  },
  "stat_arc_rep_2": {
    "main": {
      "title": "门口的瓶子不见了，换了一张字条：谢谢，别再来",
      "body": "字很工整。谢谢是真的。别再来也是真的。",
      "choice": {
        "keep": {
          "label": "把字条夹进日记",
          "log": "两句话都夹进日记：谢谢，别再来。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "你把字条揉了。"
        }
      }
    }
  },
  "stat_arc_dark_1": {
    "main": {
      "title": "半夜被楼上的脚步声吵醒",
      "body": "灯关着。半夜你被吵醒了，楼上有人在走，地板咯吱响。冰箱不转，屋里特别静，所以脚步听得格外清楚。过了几分钟眼睛才适应，你差点把墙角那团椅子的影子看成个人。",
      "choice": {
        "candle": {
          "label": "点蜡烛，把窗帘拉严",
          "log": "蜡烛点了。窗帘拉严。楼上又走了两步。"
        },
        "sit": {
          "label": "在黑里坐到不怕",
          "log": "你坐着。后来不怕了，只是眼睛酸。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "去供电表把灯排上去。"
        }
      }
    }
  },
  "stat_arc_dark_2": {
    "main": {
      "title": "窗外有人问：还亮灯吗",
      "body": "灯还是关着。窗外有人喊，问这屋还亮不亮灯。你听不清是对面楼还是楼下。你要是答了，不管亮不亮，外面的人都知道这间屋子有人。",
      "choice": {
        "no": {
          "label": "说不亮",
          "log": "你说了不亮。窗外没有第二句。"
        },
        "yes": {
          "label": "说亮，然后立刻后悔",
          "log": "你说了亮。窗帘缝外有人停了一下。"
        },
        "skip": {
          "label": "什么都不做",
          "log": "不回答。问了一会儿，外面自己停了。"
        }
      }
    }
  }
};

registerTree('event', data);
