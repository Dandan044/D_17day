import { registerTree } from '../../t';

export const data = {
  "prep_hoarding_rush": {
    "supermarket": {
      "title": "超市门口排到了马路上",
      "body": "晚上八点，你路过那家每周都去的超市，队伍从生鲜区一直排到了马路对面，有人推着三辆购物车，有人在为最后一箱矿泉水争执。\n收银员低着头一趟趟扫码装袋，对柜台外面吵吵嚷嚷的一切都不再回应。",
      "choice": {
        "queue": {
          "label": "排队，能买多少买多少",
          "note": "花 1 小时，拿到打折前的价格",
          "log": "你在超市排了两个小时，抢到了别人手慢没拿到的那些。"
        },
        "staff": {
          "label": "绕到后门找认识的理货员",
          "reason": "需要谈判 2 级",
          "log": "你从后门拿到了还没上架的货。排队的人看见了，其中有人记住了你的脸。"
        },
        "pass": {
          "label": "算了，明天再说",
          "log": "你转身走了。明天这里会更空。"
        }
      }
    },
    "gasline": {
      "title": "加油站排队时前面吵起来了",
      "body": "两个男人在为加油顺序推搡。工作人员喊了几声就退回了便利店，把门锁了。\n你的油表还有半箱。前面还有六辆车。",
      "choice": {
        "wait": {
          "label": "等着，加满",
          "log": "你排了半天终于加满了油，等待的时候听见后面有人在打电话，说着说着就哭了。"
        },
        "jerrycan": {
          "label": "同时买两个油桶灌满",
          "log": "你买了两个油桶全灌满带回家，后面排队的人当场骂了起来。"
        },
        "leave": {
          "label": "不掺和，开走",
          "log": "你掉头走了。半箱油，够跑一百公里。"
        }
      }
    }
  },
  "prep_neighbor_asks": {
    "lijie_water": {
      "title": "李姐来敲门",
      "body": "三楼的李姐站在门口，手里攥着一个空水壶。她一个人带孩子，你知道的。\n\"小区停水通知贴出来了，说是检修。我看你昨天搬了好几箱水上来……\"\n话说到一半她就停了下来，剩下的半句怎么也不好意思开口。",
      "choice": {
        "give": {
          "label": "给她 8 L 水",
          "log": "你给了李姐八升水。她说等来水了一定还你。"
        },
        "share_info": {
          "label": "不给水，但告诉她该去哪买",
          "log": "你把还有存货的便利店地址告诉了李姐，她谢了你，转身走得很快。"
        },
        "refuse": {
          "label": "说家里也不够",
          "log": "你说家里也不够，她点点头说理解，然后下楼了，你关门的时候手在门把上停了一下。"
        }
      }
    },
    "child_medicine": {
      "title": "有人在楼道里问有没有退烧药",
      "body": "二楼那家的孩子从昨晚开始发烧，三十九度二。医院的号已经排到了后天。\n孩子的父亲站在楼梯口，看谁下楼就问一句。他已经问了很多人了。",
      "choice": {
        "give_meds": {
          "label": "给他 2 组药",
          "log": "你给了那家人退烧药。孩子的父亲握着你的手说了三遍谢谢。"
        },
        "drive": {
          "label": "开车送他们去别的医院",
          "reason": "需要有车",
          "log": "你开了四十公里把他们送到了郊区的医院。回来的路上油表见底。"
        },
        "no": {
          "label": "说没有",
          "log": "你说家里没有。关上门以后，你看着柜子里那两盒药，半天没动。"
        }
      }
    }
  },
  "prep_neighbor_repay": {
    "repay": {
      "title": "有人在你门口放了东西",
      "body": "一个纸箱，上面压着一张便条：\"谢谢。用得上的话就拿着。\"没有署名。\n里面是几袋米、两包挂面、一卷胶带，还有一小袋盐。都是很实在的东西。",
      "choice": {
        "take": {
          "label": "收下",
          "log": "你把箱子搬进屋，那卷胶带当晚就用上了，正好封完厨房窗框的最后一道。"
        }
      }
    }
  },
  "prep_coalarm": {
    "hardware_tip": {
      "title": "五金店老板多说了一句",
      "body": "王老板一边给你装木板一边说：\"你要真打算封窗户，那买个这个。\"\n他从柜台底下摸出一个白色的小圆盘，巴掌大，六十八块。\"一氧化碳报警器。密封做太好，屋里烧个炉子就能出事。前年我们这条街有一家人，三口，全是这么走的。\"\n六十八块，而你今天已经花出去两千多了。",
      "choice": {
        "buy": {
          "label": "买一个（68 元）",
          "log": "你花六十八块买了个一氧化碳报警器，随手扔进了工具箱。"
        },
        "buy_two": {
          "label": "买两个，一个给邻居（136 元）",
          "log": "你买了两个，把其中一个塞进了李姐家的门缝。"
        },
        "pass": {
          "label": "不用，我又不是没常识",
          "log": "你没买。王老板耸了耸肩，把那个白圆盘放回了柜台底下。"
        }
      }
    }
  },
  "prep_iodine": {
    "pharmacy": {
      "title": "药店的碘片还剩两盒",
      "body": "柜台后的年轻店员指了指最上层：\"今天来问这个的第十一个人了。上午还有一整箱。\"\n价格是平时的六倍。碘化钾片，一盒二十片，够一个人吃十天。\n说明书上写着：核辐射暴露前或后二十四小时内服用。你不确定外面是不是已经到了那一步。",
      "choice": {
        "buy_both": {
          "label": "两盒都要",
          "log": "你买了两盒碘片。如果猜错了，这九百块就是买了个心安。"
        },
        "buy_one": {
          "label": "买一盒就够了",
          "log": "你买了一盒碘片。够一个人吃十天，前提是只有一个人。"
        },
        "pass": {
          "label": "把钱花在更可能用上的东西上",
          "log": "你没买碘片，买了两盒退烧药和一卷绷带。出门前你回头看了一眼，货架上那两盒已经没了。"
        }
      }
    }
  },
  "prep_vehicle": {
    "used_van": {
      "title": "有人在卖一辆面包车",
      "body": "小区停车场那个总在修车的男人要走了。他说要回老家，车带不走。\n十四年的旧面包车，跑了二十六万公里，排气有点响，但能跑。他要一万二，现金。\n\"你要真想活，\"他说，\"腿是不够的。\"",
      "choice": {
        "buy": {
          "label": "买（12000 元）",
          "log": "你买下了那辆面包车。油箱里还剩八升。"
        },
        "haggle": {
          "label": "压价",
          "reason": "需要谈判 3 级",
          "ok": {
            "log": "你把价格压到了八千五。他说算了，反正带不走。"
          },
          "bad": {
            "log": "他不肯让价，还多要了几百。你还是买了。"
          }
        },
        "cart": {
          "label": "买不起车，去弄一辆手推车",
          "log": "你花三百块买了辆建材市场的手推车。载重四十公斤，总比背着强。"
        },
        "pass": {
          "label": "不需要",
          "log": "你没买车。"
        }
      }
    }
  },
  "prep_gun": {
    "blackmarket": {
      "title": "旧货市场后面有人卖猎枪",
      "body": "他先问你要不要买刀，你摇头。他又看了你两秒，说跟我来。\n铁皮棚子后面，一个帆布包。一把老式猎枪，枪管有锈，还有三十发弹。\n\"两万四。别问来路，我也不问你要干什么。\"\n这东西能救你的命，也能让你在下周被人一枪打死。",
      "choice": {
        "buy": {
          "label": "买下（24000 元）",
          "log": "你带回了一把猎枪。它现在躺在衣柜最里面，用毛巾包着。"
        },
        "ammo_only": {
          "label": "只买弹药，枪不要",
          "log": "你只买了弹药，说不上为什么，反正最后还是买了。"
        },
        "report": {
          "label": "走开，然后报警",
          "log": "你报了警。接线员的声音很累，说会记录。你不知道会不会有人来。"
        },
        "walk": {
          "label": "什么都不做",
          "log": "你走开了。"
        }
      }
    }
  },
  "prep_family_call": {
    "mother": {
      "title": "你妈打来电话",
      "body": "\"新闻里说的那个事，是真的吗？\"\n她在两百公里外的县城，一个人住。你听得见她那边电视的声音。\n\"要不我过去？\"她问，\"还是你回来？\"\n你手里正拿着一卷密封胶带。",
      "choice": {
        "come_here": {
          "label": "让她来，你去接",
          "reason": "需要有车才能去接",
          "log": "你开了四小时把她接过来了。她带了两个装满腌菜的坛子和一整包药。"
        },
        "stay": {
          "label": "让她留在县城，那里人少",
          "log": "你让她别动，说县城人少更安全。挂电话前她说了句\"你自己当心\"。"
        },
        "lie": {
          "label": "说都是谣言，让她别担心",
          "log": "你说都是谣言。她信了，还笑了。你挂了电话，继续贴胶带。"
        }
      }
    }
  },
  "prep_workplace": {
    "boss": {
      "title": "公司群里发了通知",
      "body": "\"针对近期网络传言，公司提醒各位员工不要轻信谣言、不要传播恐慌。明日起正常上班，无故缺勤按旷工处理。\"\n下面有三十个\"收到\"。你盯着输入框。\n你还有四天。",
      "choice": {
        "go": {
          "label": "去上班",
          "note": "拿到这个月的工资",
          "log": "你去上班了。整个办公室没有人提这件事，所有人都在刷手机。"
        },
        "quit": {
          "label": "辞职，把时间全用在准备上",
          "log": "你发了辞职信，没等回复。从今天起你只有一件事要做。"
        },
        "warn": {
          "label": "在群里把你知道的都说了",
          "log": "你在群里把你查到的都发了。两个人私聊你说谢谢，其他人当你疯了。HR 让你注意言论。"
        }
      }
    }
  },
  "prep_scam": {
    "kit": {
      "title": "有人在小区门口卖应急包",
      "body": "一辆三轮车，一块手写纸板：\"家庭应急生存包 998 元/套 限量\"。\n包里能看见压缩饼干、一件雨衣、一个手电、几根蜡烛。你估算了一下，成本不到一百八。\n围着看的人不少，已经有两个在扫码。",
      "choice": {
        "expose": {
          "label": "当众算给大家听",
          "log": "你把每样东西的市价当众报了一遍。卖的人收摊走了，有个大姐拉着你问该买什么。"
        },
        "buy_cheap": {
          "label": "压价买两套（自己拆用）",
          "ok": {
            "log": "你花九百拿了两套，拆开发现手电还挺好用。"
          },
          "bad": {
            "log": "他一分钱都不让，你还是买了两套，回家拆开的时候有点后悔。"
          }
        },
        "ignore": {
          "label": "不管，走了",
          "log": "你走开了。身后有人在扫码付款。"
        }
      }
    }
  },
  "prep_pet": {
    "dog": {
      "title": "楼道里有一条狗",
      "body": "中华田园犬，一岁多，脖子上有项圈但没牌子。对门那家昨天搬走了。\n它没有叫，只是在他们家门口趴着，看见你上楼就抬了下头。\n养它要吃粮，会叫，会招来注意。但狗的耳朵比你好用得多。",
      "choice": {
        "take": {
          "label": "带它回家",
          "log": "你把它带回了家，它一进屋就趴在门口，耳朵一直朝着楼梯的方向。"
        },
        "feed": {
          "label": "喂点东西，但不带走",
          "log": "你放了一碗东西在它面前，然后关上了自己的门。"
        },
        "ignore": {
          "label": "你养不起",
          "log": "你从它旁边走过去了。第二天它还在那儿，第三天就不在了。"
        }
      }
    }
  },
  "prep_evacuate": {
    "convoy": {
      "title": "有人组了车队要出城",
      "body": "业主群里有人发起了自驾出城，说是去西边山区的农家乐包场，已经有十一户报名。\n出发时间是明天早上六点。他们要每户三千块，用于统一采购。\n\"留在城里就是等死。\"发起人这么说。他可能是对的，也可能只是害怕。",
      "choice": {
        "join": {
          "label": "报名（3000 元）",
          "reason": "需要车和 3000 元",
          "log": "你报了名交了三千块，结果一整晚都在犹豫明天要不要真的跟着走。"
        },
        "stay": {
          "label": "不去，你已经决定守在这里",
          "log": "你没报名。这栋楼里你最熟，而路上的事没人说得清。"
        },
        "warn": {
          "label": "劝他们别走，路上更危险",
          "ok": {
            "log": "你把交通管制和路况一条条说了。有六户退出了车队，留下来跟你一起加固楼门。"
          },
          "bad": {
            "log": "你说了很多，但没人听。发起人说你是在散播恐慌。"
          }
        }
      }
    }
  },
  "prep_stranger_generator": {
    "generator": {
      "title": "有人要转让一台发电机",
      "body": "同城二手平台，五分钟前发布：柴油发电机 5 kW，九成新，一万六，只接受面交现金。\n照片看着是真的。但地址在城东的一个仓库区，而且他说\"必须今晚\"。\n这可能是清库存的商家，也可能是三个人拿着撬棍在等你。",
      "choice": {
        "go_alone": {
          "label": "带钱去",
          "ok": {
            "log": "是真的。你把那台机器搬回来，用了两趟。"
          },
          "bad": {
            "log": "仓库里只有三个人和一根钢管。钱没了，你的左臂被划开一道口子。"
          }
        },
        "go_daylight": {
          "label": "坚持改成白天、换公共场所",
          "ok": {
            "log": "你坚持改到白天在派出所门口交易。他答应了，还多要了一千五。机器是真的。"
          },
          "bad": {
            "log": "对方直接把你拉黑了。你不知道是躲开了一个骗局，还是错过了一台发电机。"
          }
        },
        "pass": {
          "label": "不去",
          "log": "你没去。十分钟后帖子删了。"
        }
      }
    }
  },
  "prep_landlord": {
    "property": {
      "title": "物业在楼道贴了告示",
      "body": "\"关于近期部分住户擅自改造门窗、堆放杂物的整治通知：请于三日内自行恢复原状，否则将强制清理并追究责任。\"\n下面有人用笔写了一行：\"你们清理试试。\"\n你的窗户已经钉上了木板。",
      "choice": {
        "bribe": {
          "label": "找物业主任私下沟通",
          "log": "你请物业主任抽了根烟，塞了个红包。他说他什么也没看见。"
        },
        "organize": {
          "label": "联合其他住户一起顶回去",
          "ok": {
            "log": "你把七户人家凑到了楼道里。物业撤了通知，还有人把自家剩的木板给了你。"
          },
          "bad": {
            "log": "没有一户人家愿意出面，还有个大爷指着你说，你就是那个带头搞事的。"
          }
        },
        "comply": {
          "label": "先拆掉，等等再说",
          "log": "你把木板拆下来靠在阳台角落，三天后还得花上两倍的工夫才能重新钉回去。"
        }
      }
    }
  },
  "prep_intel_conflict": {
    "two_reports": {
      "title": "两份通报互相矛盾",
      "body": "一份是市级的，说\"生产生活秩序正常，请勿传播不实信息\"。\n另一份是同一天下午区级发的，要求\"各单位做好应急物资储备并上报库存\"。\n它们同时挂在两个官方账号上，谁也没删。",
      "choice": {
        "analyze": {
          "label": "交叉比对，找出哪份更可信",
          "ok": {
            "log": "你把两份通报的落款单位、发布时间和往年同类文件对了一遍。你现在更清楚该信什么。"
          },
          "bad": {
            "log": "你看了两个小时，唯一的结论是：他们自己也不知道。"
          }
        },
        "trust_local": {
          "label": "信区级那份，按最坏情况准备",
          "log": "你决定信区级那份。按最坏情况开始备货。"
        },
        "ignore": {
          "label": "不看了，去干活",
          "log": "你关掉手机去搬东西，先把眼前能搬的搬完。"
        }
      }
    }
  }
};

registerTree('event', data);
